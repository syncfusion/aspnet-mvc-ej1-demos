...
{
    public class DrillThroughController : ApiController
    {
        ...
        static string connectionString = ConfigurationManager.ConnectionStrings["Adventure Works DW1"].ConnectionString + "locale identifier=" + cultureIDInfoval + ";";
        ...
        public Dictionary<string, object> DrillThroughDataTable(Dictionary<string, object> jsonResult)
        {
            OlapDataManager DataManager = new OlapDataManager(connectionString);
            DataManager.SetCurrentReport(OLAPUTILS.Utils.DeserializeOlapReport(jsonResult["currentReport"].ToString()));
            return olapClientHelper.DrillthroughDataTable(DataManager, "normal", jsonResult["cellPos"].ToString(), jsonResult["selector"].ToString());
        }
        ...
        private OlapReport CreateOlapReport(string Report)
        {
            OlapReport olapReport = new OlapReport() { Name = "Default Report" };
                 ...
                 measureElement.Elements.Add(new MeasureElement { UniqueName = "[Measures].[Sales Amount Quota]", Format=Syncfusion.Olap.Reports.NumberFormat.Currency});
