$(function () {
    ...
    $("#doctorsdeptlist").ejListBox({
        ...
        fields: { text: "Text" },
        checkChange: "onListCheck",
        template: "<div id='doctors'><div id='dtrimage'><img class='doctorimg' src='../../content/images/appointmentplanner/${Text}.png' alt='doctor'/></div><div id='dtrdetails'><div class='doctorname'>${Name}</div><div class='designation'>${Designation}</div></div><div id='dtrcategory'><i class='icon-${Value}'></i></div></div>"
    });
    $("#upcomming").ejListBox({
        width: "100%",
        height: "100%",
...
function _appointImages(id) {
    ...
    switch (resText) {
        case "":
            return null;
        default:
            return "<img src='../../content/images/appointmentplanner/" + resText + ".png' style='width:40px;height:40px;border-radius:50%;'/>";
    }
}
...
function _resourceImages(id) {
    ...
    switch (resText) {
        case "":
            return null;
        default:
            return "<img src='../../content/images/appointmentplanner/" + resText + ".png' style='width:25px;height:20px;float:left;border-radius:50%;margin-right:10px;'/>";
    }
}
...
function _getResources(id, type) {
    ...
    if (type == "image") {
        var resObj = new ej.DataManager(window.doctorName).executeLocal(new ej.Query().where(schObj.model.resources[schObj.model.resources.length - 1].resourceSettings["id"], ej.FilterOperators.equal, id));
        if (resObj.length != 0) var resText = resObj[0].Text;
        result = "<img src='../../content/images/appointmentplanner/" + resText + ".png' style='width:50px;height:50px;float:left;border-radius:50%;'/>";
    }
    else if (type == "room") {
        ...
        if (columnName != undefined) {
            ...
            if (columnName != "" && obj[columnName] == null) {
                ...
                if (formelement[index].type == "hidden") {
                    var resdata = (columnName == "Department") ? window.departmentName : window.doctorName;
                    var dptId = new ej.DataManager(resdata).executeLocal(new ej.Query().where("Text", ej.FilterOperators.equal, value));
                    if (dptId.length > 0)
                        (columnName == "Department") ? obj["RoomId"] = dptId[0].Id : obj["OwnerId"] = dptId[0].Id;
                }
                ...
                if (columnName != "Department_hidden" && columnName != "deptdoctors" && columnName != "deptdoctors_hidden" && columnName != "patientadd")
                    obj[columnName] = value;
            }
        }
    }
