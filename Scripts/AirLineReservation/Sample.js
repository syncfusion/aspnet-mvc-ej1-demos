...
function onDiagramCreation(args) {
...
function addAdultIcon() {
    ...
    img.setAttribute("src", "../../Images/AirLineReservation/icon_Adult.png");
...
function addChildIcon() {
    ...
    img.setAttribute("style", "margin-top:10px;margin-left:10px;border:2px solid lightblue");
    img.setAttribute("src", "../../Images/AirLineReservation/icon_Child.png");
    img.setAttribute("draggable", "true");
...
function filterByPrice(sender) {
    ...
    var gridObj = $("#FlightTicketBooking").data("ejGrid");
    if (gridObj.model.dataSource.length > 0)
        $("#btnOpenPopup").css("display", "block");
    else
...
function showFrontLayer() {
    ...
    document.getElementById('frontlayer').style.display = 'block';
...
function hideFrontLayer() {
    ...
    document.getElementById('frontlayer').style.display = 'none';
...
function onNodeClick(sender) {
    if (sender.element && sender.element.type == "image") {
        ...
        if (selectedNode && (selectedNode.addInfo == undefined || selectedNode.addInfo == "" || Object.keys(selectedNode.addInfo).length == 0)) {
            var imageSrc = "";
            var adultAdded = false;
            var childAdded = false;
            if (adultCount > 0) {
                adultAdded = true;
                imageSrc = "../../Images/AirLineReservation/icon_Adult.png";
            } else if (childCount > 0) {
                childAdded = true;
                imageSrc = "../../Images/AirLineReservation/icon_Child.png";
            }
            if (adultCount > 0 || childCount > 0) {
                var diagram = $("#diagramContent").ejDiagram("instance");
                //if (selectedNode instanceof ej.Diagram.Node) {
...
function onItemDrop(args) {
    ...
    if (element && element.shape && element.shape == "rectangle") {
        ...
        if (selectedNode && Object.keys(selectedNode.addInfo).length == 0) {
            selectedNode.addInfo = "";
        }
        if (selectedNode && selectedNode.fillColor === 'red') {
            selectedNode.addInfo = "booked";
        }
        ...
        if (selectedNode && (selectedNode.addInfo == undefined || selectedNode.addInfo == "" || !jQuery.isEmptyObject(selectedNode))) {
            ...
            //if (selectedNode instanceof ej.Diagram.Node) {
            if (!selectedNode.line && selectedNode.type != "group") {
                diagram.updateNode(selectedNode.name, { shape: "rectangle" });
                if (selectedNode.addInfo == "AdultSeat") {
                    addAdultIcon();
                    adultCount = adultCount + 1;
                }
            ...
            var seatName = selectedNode.name.toString();
            seatName = seatName.replace("seat", "Seat ");
            alert("Selected " + seatName + " is already booked");
        }
        updateAdultText();
        updateChildText();
    }
